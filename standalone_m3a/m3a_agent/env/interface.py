...
from m3a_agent.env import actuation
...
class AsyncEnv(abc.ABC):
  ...
  def get_state(self, wait_to_stabilize: bool = False) -> State:
    ...
    Returns:
      Observation containing RGB array of screen and UI elements derived from
        the UI hierarchy.
    """
...
class AsyncAndroidEnv(AsyncEnv):
  """Async environment interface using ADB to communicate with device."""
  ...
  def reset(self, go_home: bool = False) -> State:
    if go_home:
      adb_utils.press_home_button(self.controller.adb, display_id=self._display_id)
    self.interaction_cache = ''
    return self._get_state()

  def _get_state(self):
    pixels = self.controller.get_screenshot()
    ui_elements = self.controller.get_ui_elements()
    return State(pixels=pixels, ui_elements=ui_elements, auxiliaries={})
  ...
  def execute_action(self, action: json_action.JSONAction) -> None:
    if action.action_type == json_action.ANSWER:
      ...
      return
    if action.action_type == json_action.SPEAK:
      self.interaction_cache = action.text
      if action.text:
        self.display_message(action.text, header='Agent:')
      return
    if action.action_type == json_action.ASK:
      self.interaction_cache = action.text
      if action.text:
        self.display_message(action.text, header='Agent asks:')
      return
  ...
  def hide_automation_ui(self) -> None:
    ...
    adb_utils.issue_generic_request(
        'shell settings put system pointer_location 0', self.controller.adb
    )
  ...
  @property
  def foreground_activity_name(self) -> str:
    activity = adb_utils.get_current_activity(self.controller.adb)[0]
    if activity:
      return activity
    else:
      return ''
  ...
  @property
  def logical_screen_size(self) -> tuple[int, int]:
    return adb_utils.get_logical_screen_size(self.controller.adb)
  ...
  @property
  def orientation(self) -> int:
    return adb_utils.get_orientation(self.controller.adb)

  @property
  def physical_frame_boundary(self) -> tuple[int, int, int, int]:
    return adb_utils.get_physical_frame_boundary(self.controller.adb)
