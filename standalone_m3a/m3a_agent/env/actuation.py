  ...
  if action.action_type in ['click', 'double_tap', 'long_press']:
    ...
    if idx is not None:
      ...
      if action.action_type == 'click':
        adb_utils.tap_screen(x, y, env, display_id=display_id)
      elif action.action_type == 'double_tap':
        adb_utils.double_tap(x, y, env, display_id=display_id)
      else:
        adb_utils.long_press(x, y, env, display_id=display_id)
    ...
    if text:
      if action.index is not None or (
        ...
        execute_adb_action(click_action, screen_elements, screen_size, env, display_id=display_id)
        time.sleep(1.0)

      if action.clear_text:
        # Select all existing text (Ctrl+A) and delete it (backspace).
        prefix = adb_utils._input_cmd_prefix(display_id)
        adb_utils.issue_generic_request(
            prefix + ['keycombination', '113', '29'],
            env,
        )
        adb_utils.issue_generic_request(
            prefix + ['keyevent', '67'],
      ...
      adb_utils.type_text(text, env, timeout_sec=10, display_id=display_id)
      adb_utils.press_enter_button(env, display_id=display_id)
    else:
      logging.warning(
    ...
    adb_utils.press_home_button(env, display_id=display_id)

  elif action.action_type == 'navigate_back':
    adb_utils.press_back_button(env, display_id=display_id)

  elif action.action_type == 'press_keyboard':
    adb_utils.press_keyboard_generic(action.keycode, env, display_id=display_id)
    ...
    if app_name:
      adb_utils.launch_app(app_name, env, display_id=display_id)
    else:
    ...
    if action.activity_nickname == 'app_drawer':
      ...
      request = adb_utils.generate_swipe_command(start_x, start_y, end_x, end_y, display_id=display_id)
...
def find_and_click_element(
  ...
  execute_adb_action(action, ui_elements, screen_size, env.adb)
